...
////// Specular:
                ...
                float3 diffuseColor = (_Montain_var.rgb+(((_MainTex_var.rgb+_Moon_var.rgb)+_Clouds_var.rgb)*_node_8442_var.rgb)); // Need this for specular when using metallic
...
////// Emissive:
                float3 emissive = (((_Clouds_var.rgb*_IntensityCloud)+((_Moon_var.rgb*(1.0 - (_Clouds_var.rgb*_IntensityShadowCloud)))*_IntensityMoon))*_node_8442_var.rgb);
/// Final Color:
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _Moon_var = tex2D(_Moon,TRANSFORM_TEX(i.uv0, _Moon));
                float4 _node_8442_var = tex2D(_node_8442,TRANSFORM_TEX(i.uv0, _node_8442));
                o.Emission = (((_Clouds_var.rgb*_IntensityCloud)+((_Moon_var.rgb*(1.0 - (_Clouds_var.rgb*_IntensityShadowCloud)))*_IntensityMoon))*_node_8442_var.rgb);
                
                float4 _Montain_var = tex2D(_Montain,TRANSFORM_TEX(i.uv0, _Montain));
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 diffColor = (_Montain_var.rgb+(((_MainTex_var.rgb+_Moon_var.rgb)+_Clouds_var.rgb)*_node_8442_var.rgb));
                float specularMonochrome;
                float3 specColor;
